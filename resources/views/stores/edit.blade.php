...
@section('title', 'Edit Store - Store Management')
        ...
        <div class="px-8 py-6 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-gray-600 mt-1">Update store information for {{ $store->store_name }}</p>
                </div>
            </div>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('stores.update', $store) }}" class="p-8 space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="state" 
                            ...
                            @foreach($states as $code => $name)
                                <option value="{{ $code }}" {{ old('state', $store->state) === $code ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center space-x-3 cursor-pointer group">
                                <input type="radio" 
                                       name="designation" 
                                       value="primary"
                                       {{ old('designation', $store->designation) === 'primary' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center space-x-3 cursor-pointer group">
                                <input type="radio" 
                                       name="designation" 
                                       value="alternate"
                                       {{ old('designation', $store->designation) === 'alternate' ? 'checked' : '' }}
                                       class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500">
                                <span class="text-gray-700 group-hover:text-gray-900 transition-colors">
                                    Alternate Store
                                </span>
